...
int __fastcall BapdpRegisterBitlockerStatus(int result, int a2)
{
  ...
  if ( result )
  {
    if ( a2 == 4 )
    {
      ...
      if ( result >= 0 )
      {
        RtlInitUnicodeString((unsigned int)v2, L"BitlockerStatus");
        v3 = 24;
        v4 = 0;
        v5 = v2;
        v6 = 576;
        ...
        {
          ZwClose();
          RtlInitUnicodeString((unsigned int)v2, L"BootStatus");
          ZwSetValueKey();
        }
        result = ZwClose();
      }
